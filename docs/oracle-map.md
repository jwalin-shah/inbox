...
| MCP control plane (`mcp_control_plane.py`) | `saltzer-schroeder-oracle.md` | `api-design-oracle.md` | Fail-closed auth, complete mediation via ApprovalStore lookup, ingest-only spawn=0; fifteen-tool canonical surface (the original ten, plus `execution_submit`/`execution_status`/`execution_events` ExecutionIntent seam, plus `execution_inspect`/`execution_start`/`execution_run_status`/`execution_cancel`/`execution_verify_close` from `execution_adapter.py`, registered on the same server); FastMCP Streamable HTTP lifespan/session init |
| Execution adapter (`execution_adapter.py`) | `saltzer-schroeder-oracle.md` | `fowler-oracle.md` | Fail-closed authority check before spawn, complete mediation via `AdmissionAuthority`, zero-spawn default (`MissingAuthorityInterface`); the five tools it defines are registered on `mcp_control_plane`'s canonical surface, not a second server; fixed-argv adapter pattern for Orca CLI, mirroring `bridge_work_client.py` |
