...
| iMessage lookup | `GET /conversations?source=imessage`, `GET /messages/imessage/{conv_id}`, `POST /search` with `connector:imessage` | `list_message_threads`, `get_message_thread`, `search_personal_data` | Inbox's native local Messages database reader; rows carry source, conversation/message id, sender, timestamp. Optional `imsg` CLI is not required for this path. |
