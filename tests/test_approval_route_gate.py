...
MUTATING_METHOD_EXCEPTION_POLICY = {
    ...
    ("POST", "/events/backfill/index"): ApprovalExceptionPolicy(
        "local_write", True, "local evidence backfill; no provider write"
    ),
    ("POST", "/triage/messages"): ApprovalExceptionPolicy(
        "pure_read", True, "read-only message triage projection; no provider write"
    ),
    ("POST", "/maps/route"): ApprovalExceptionPolicy(
        "pure_read", True, "read-only multi-stop route calculation; no provider write"
    ),
